...
\label{sec_technical_lemmas}

This section contains technical lemmas essential for the analysis presented in the main article.

\subsection{Technical lemmas for general domain alignment}
\label{ssec_technical_lemmas_general}
...
\begin{lemma}
  ...
  \end{split}
  \end{equation*}
  Then the expected target loss is bounded as $\Lt(\ft, \h) \leq   \Lw(\fs, \ft, \h)+ (1-\alpha) \LLs  \D(\fs, \ft)$.
\end{lemma}
...
&\leq \alpha \Lt(\ft, \h) +  (1-\alpha) \left( \Ls (\fs, \h) + \LLs \ \D(\fs, \ft) \right) \\
&= \Lw(\fs, \ft, \h)+ (1-\alpha) \LLs  \D(\fs, \ft)
\end{align*}
using \cref{assum_existence_LLs}.
\end{proof}

\begin{lemma}
\label{lem:weight_loss_gen_supp}
Let the conditions in Assumption 2.2 hold. Let $\hLw(\fs, \ft, \h) \triangleq  (1-\alpha) \hLs(\fs, \h) + \alpha \hLt(\ft, \h)$ denote the empirical weighted loss. Then, we have
\begin{equation}
\begin{split}
  & P\Bigg\{\sup_{\fs \in \Fs,\, \ft \in \Ft,\, \h \in \Hs} 
     \big|  \Lw(\fs, \ft, \h) - \hLw(\fs, \ft, \h) \big| \leq \epsilon \Bigg\} \\
  &\qquad \geq 1 
    - 2\, \N\left( \Hs \circ \Ft, \frac{\epsilon}{8\alpha\Lls}, \dt \right)\exp\left(-\frac{\Mt \epsilon^2}{8\alpha^2 \bls^2}\right) \\
  &\qquad \quad - 2\, \N\left( \Hs \circ \Fs, \frac{\epsilon}{8(1-\alpha)\Lls}, \ds \right)\exp\left(-\frac{\Ms \epsilon^2}{8(1-\alpha)^2 \bls^2}\right).
\end{split}
\end{equation}
\end{lemma}

\begin{lemma}
\label{lem:bernstein_src_trg_supp}
Let the source and target distributions and the transformations $\fs: \Xs \rightarrow \X$ and $\ft: \Xt \rightarrow \X$ be such that Assumption 2.3 holds. Also, for given $\epsilon>0$, let the number of source and target samples be such that $\Ns > \frac{\vars}{\epsilon^2}, \quad \Nt > \frac{\vart}{\epsilon^2}.$
Then for the source domain we have
\begin{equation}
\label{eq:lem_src_empmean_supp}
\begin{split}
...
\begin{lemma}
\label{lem:unif_bnd_D_hD_supp}
Let Assumptions 2.3, 2.4 hold. Given $\epsilon>0$, let the number of source and target samples be such that $\Ns > \frac{16 \vars}{\epsilon^2}, \quad \Nt > \frac{16 \vart}{\epsilon^2}.$
Let us define the functions
$\as(\Ns, \epsilon) \triangleq 
  \frac{1}{8}\left( \frac{\sqrt{\Ns} \epsilon}{4 \sigma_s} -1\right)^2 \frac{1}{1+\left( \frac{\sqrt{\Ns} \epsilon}{4 \sigma_s} -1 \right)\frac{\Cs}{2 \sqrt{\Ns} \sigma_s }}, \quad
...
\right) \geq 1 - \N(\Fs, \frac{\epsilon}{8}, \dXs) \exp(-\as(\Ns, \epsilon)) 
- \N(\Ft, \frac{\epsilon}{8}, \dXt) \exp(-\at(\Nt, \epsilon)).$
\end{lemma}

\begin{lemma}
\label[lemma]{lem_ddan_hddan_dev_supp}
Let Assumption 3.11 hold. Assume also that the composite function classes $\Vs$ and $\Vt$ are compact with respect to the metrics 
$\dVs(\vs_1, \vs_2)  \triangleq \sup_{\xs \in \Xs}  | \vs_1(\xs) - \vs_2(\xs) |, \quad \dVt(\vt_1, \vt_2)  \triangleq \sup_{\xt \in \Xt}  | \vt_1(\xt) - \vt_2(\xt) |$
...
\subsection{Technical lemmas for domain-adaptive neural networks}
\label{ssec_technical_lemmas_neural}

\subsection{Measurability of feature mappings}

\begin{lemma}
\label[lemma]{lem_fs_ft_measble_supp}
Let the condition in Assumption 3.3 hold. Then the mappings $\fsl: \Xs \rightarrow \Xl$ and $\ftl:  \Xt \rightarrow \Xl$ for $\lay=1, \dots, \numL-1$, and the mappings $\fs:  \Xs \rightarrow \X $ and $\ft:  \Xt \rightarrow \X $ are measurable. Moreover, assuming that $E[\sqrt{\krl(\hsl, \hsl)}]<\infty$ and $E[\sqrt{\krl(\htl, \htl)}]<\infty$, the functions $E[\fsl(\xs)]: \Rdl \rightarrow \R$ and $E[\ftl(\xt)]: \Rdl \rightarrow \R$ defined as
%
\begin{equation*}
\begin{split}
...
The proof of Lemma \ref{lem_fs_ft_measble_supp} is given in Appendix \ref{pf_lem_fs_ft_measble}.

\subsection{Compactness of function classes}

\begin{lemma}
\label[lemma]{lem_Fs_Ft_HFs_HFt_comp_supp}
Let Assumptions 3.1-3.4 hold. Then, the transformation function classes $\Fs, \Ft$ in (3.4) and the composite function classes $\Gs, \Gt$ in (3.5) are compact metric spaces, respectively under the metrics $\dXs, \dXt$ in (2.6), and the metrics $\ds, \dt$ in (2.3).
\end{lemma}

The proof of Lemma \ref{lem_Fs_Ft_HFs_HFt_comp_supp} is presented in Appendix \ref{pf_lem_Fs_Ft_HFs_HFt_comp}.

\subsection{Covering numbers of transformation classes}

\begin{lemma} 
\label[lemma]{lem_cov_num_Fs_Ft_supp}
Let Assumptions 3.1, 3.4, 3.5 hold. Then, the covering numbers of the function classes $\Fs$ and $\Ft$ are upper bounded as 
%
\begin{equation*}
\begin{split}
...
%
under condition  (3.3)
and $\Bdiml \triangleq\Beta \sqrt{\dl} $ under condition (3.2) for $\lay=2, \dots, \numL-1$, where $\Bdimz \triangleq \Binp$ and $\Bdimone \triangleq \Bopeta  \Bnet \sqrt{\done \dz}  \Binp +  \Bopeta \Bnet \sqrt{\done}$. 
%
\end{lemma} 

Lemma \ref{lem_cov_num_Fs_Ft_supp} is proved in Appendix \ref{pf_lem_cov_num_Fs_Ft}.

\subsection{Covering numbers of composite hypothesis classes}

\begin{lemma} 
\label[lemma]{lem_cov_num_HFs_HFt_supp}
Let Assumptions 3.1, 3.4, 3.5 hold. Then, the covering numbers of the function classes $\Hs \circ \Fs$ and $\Hs \circ \Ft$ are upper bounded as 
%
\begin{equation*}
\begin{split}
...
Lemma \ref{lem_cov_num_HFs_HFt_supp} is proved in Appendix \ref{pf_lem_cov_num_HFs_HFt}.
...
\label[appendix]{pf_lem_weight_loss_gen}
...
\label[appendix]{pf_lem_bernstein_src_trg}
...
\label[appendix]{pf_lem_unif_bnd_D_hD}
...
\label[appendix]{pf_lem_fs_ft_measble}
...
\label[appendix]{sec_app_lip_nonlinact}
...
\label[appendix]{pf_lem_Fs_Ft_HFs_HFt_comp}
...
\label[appendix]{pf_lem_cov_num_Fs_Ft}
...
\label[appendix]{pf_lem_cov_num_HFs_HFt}
...
\label[appendix]{pf_cor_covnum_rate}
...
\label[appendix]{pf_thm_main_result_da_mmd}
...
\label[appendix]{pf_lem_ddan_hddan_dev}
...
\label[appendix]{pf_thm_main_result_dann}
