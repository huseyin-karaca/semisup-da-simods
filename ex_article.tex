...
We can now present the following result that bounds the deviation between the expected and  empirical weighted losses. \footnote{\textcolor{blue}{Bu kısmı tamamen revwerite edip kaldıralım bence.} By \cref{lem:weight_loss_gen}, the deviation between the expected and empirical weighted losses can be bounded in terms of the covering numbers of the composite hypothesis classes and the number of labeled samples. The proof is provided in \cref{pf_lem_weight_loss_gen}. We can now combine \cref{lem:dev_exptar_expweigh,lem:weight_loss_gen} to bound the expected target loss in terms of the empirical weighted loss and the distribution distance in the following main result.}
%
\begin{theorem}
...
The expected deviations of the random variables $\{ \fs(\xis) \}_{i=1}^{\Ns}$ and $\{ \ft(\xjt) \}_{j=1}^{\Nt}$ from their means $E[\fs(\xs)]$ and $E[\ft(\xt)]$ are bounded; that is, there exist constants $\vars$ and $\vart$ satisfying
%
\begin{align}
...
By \cref{lem:bernstein_src_trg}, the deviation between the sample mean and the expectation of the source and target samples transformed to the shared Hilbert space $\X$ can be upper bounded with probability improving at an exponential rate with $\Ns$ and $\Nt$.\footnote{The proof is provided in Appendix \ref{pf_lem_bernstein_src_trg}.} Using this result, \cref{lem:unif_bnd_D_hD} establishes a uniform upper bound on the deviation $| \D(\fs, \ft) - \hD (\fs, \ft)  |$ between the expected and empirical MMD distances, which holds for all functions in the transformation function classes $\Fs$ and $\Ft$ under the compactness assumption below.\footnote{The proof is provided in Appendix \ref{pf_lem_unif_bnd_D_hD}.}
...
We are now in a position to extend \cref{thm:gen_defect_target} in the following result, where we bound the expected target loss in terms of the empirical MMD measure $\hD( \fs, \ft)$. 
...
\end{equation}
\footnote{The proof follows from \cref{thm:gen_defect_target} and \cref{lem:unif_bnd_D_hD} by the union bound.}
%
\end{theorem}
...
The Lipschitz continuity condition \eqref{eq_assum_kernel_lip_cont} holds for many widely used kernels such as Gaussian kernels. As for condition \eqref{eq_Lipcont_act}, the Lipschitz constants of the commonly used rectified linear unit, softmax and softplus activation functions are derived in \cref{sec_app_lip_nonlinact} of the supplement. Under these assumptions, the transformation function classes $\Fs, \Ft$ and the composite function classes $\Gs$, $\Gt$ are compact metric spaces with respect to the metrics defined in \eqref{eq_defn_dXs_dXt} and \eqref{eq_defn_ds_dt}, respectively. This compactness result (\cref{lem_Fs_Ft_HFs_HFt_comp}
%with proof in \cref{pf_lem_Fs_Ft_HFs_HFt_comp} 
of the supplement) is established by showing that the bounded parameter space is compact and the mapping from parameters to functions is continuous. Having established compactness, we can now characterize the covering numbers of these function classes.

To upper bound the covering numbers, we construct finite covers by discretizing the network parameter space into regular grids and leveraging the Lipschitz continuity of network components to control the induced function space distances. This analysis yields explicit covering number bounds in terms of network depth, width, and the problem-dependent constants (\cref{lem_cov_num_Fs_Ft,lem_cov_num_HFs_HFt} 
%with \cref{pf_lem_cov_num_Fs_Ft,pf_lem_cov_num_HFs_HFt} % bunu açınca Proof numaralarını de SM haline getirmek lazım, iş uzamasın diye böyle bıraktım.
of the supplement). From these technical results, we obtain the following characterization of the growth rates of covering numbers with network depth and width.
...
\begin{equation*}
 ...
 \right |
\end{equation*}
 in \cref{lem_ddan_hddan_dev}, which is the counterpart of \cref{lem:unif_bnd_D_hD} in the domain-adversarial setting.\footnote{The proof is presented in \cref{pf_lem_ddan_hddan_dev}.} Before stating the main result of this section, we formalize the following conditions. 
...
In this section, we present experimental results for the verification of the proposed generalization bounds. We study the generic bounds using shallow classifier models, then examine the sample complexity of domain-adaptive neural networks. Complete experimental details are provided in \cref{sec_detailed_exp_results} of the supplement.


\subsection{General domain alignment methods} 
\label{ssec_exp_gen_da}

We validate our findings on a synthetic data set with two classes.\footnote{Source and target data are generated by applying different geometric transformations to 400 samples from the standard normal distribution in $\R^2$. We emulate a setting where the transformations $\fs$ and $\ft$ are learnt with some estimation error $\tau$. The classifier is a regularized ridge regression trained in the common domain. Target misclassification rates are evaluated over 1000 test samples. Complete setup is provided in \cref{ssec_exp_gen_da_detailed}.}



\cref{fig:toy_error} confirms our theoretical predictions: target error decreases at rate $O(\sqrt{1/\Mt})$ with labeled target samples (panel a), larger $\Mt$ values favor larger $\alpha$ values supporting $\alpha = O(\sqrt{\Mt})$, and misclassification rate increases linearly with transformation estimation error $\tau$ proportional to $\D(\fs, \ft)$ (panel b).



We next experiment on the MIT-CBCL face data set \cite{MITCBCL} (\cref{fig:face_dataset}).\footnote{The dataset contains 3240 synthetic face images of 10 subjects under different poses and illumination conditions. We use Pose 1 (frontal) as source and Poses 2, 5, 9 as targets in separate trials. Domain alignment uses the PCA-based method of \cite{FernandoHST13} and an SVM classifier. Complete setup in \cref{ssec_exp_gen_da_detailed}.} \cref{fig:mit_error} shows misclassification rates decrease at rates $O(\sqrt{1/\Mt})$ and $O(\sqrt{1/\Ms})$ with increasing labeled samples, confirming our theory. 
...
We experimentally verify our sample complexity results (Theorems \ref{thm_main_result_da_mmd}, \ref{thm_main_result_dann}) using MNIST $\rightarrow$ MNIST-M experiments \cite{mnist,mnistm}.\footnote{MNIST (60000 images) serves as source and MNIST-M (59000 colored background images) as target. Networks are trained with varying numbers of labeled and unlabeled samples. Hyperparameters are chosen to maintain overfitting regime for sample complexity characterization. Complete details in \cref{ssec_exp_dan_detailed}.}

\subsubsection{MMD-based domain adaptation networks}
\label{ssec_exp_mmd_dan}


We adopt the MMD-based architecture from \cite{LongCWJ15}.\footnote{The network uses convolutional layers followed by fully connected MMD layers with coupled parameters, minimizing cross-entropy loss plus MMD distance across all layers. In \cref{fig_mmd_results_L}, we vary the number $\numL$ of MMD layers in overfitting regime. Left panels show accuracy vs $\numL$ for different sample sizes; right panels plot minimum samples required to achieve reference accuracy, with fitted quadratic polynomials.} Figures \ref{fig_mmd_results_L}-\ref{fig_mmd_results_d} demonstrate the predicted quadratic growth $\Ms, \Ns=O(\numL^2)$ and $\Ms=O(\dcom^2)$ with respect to network depth and width. 



\cref{fig_mmd_results_alpha} confirms the theoretical prediction that $\alpha_{opt}=O(\sqrt{\Mt})$ from \cref{thm_main_result_da_mmd}.
...
We adopt the domain-adversarial architecture from \cite{GaninUAGLLML16}.\footnote{The architecture trains a feature extractor and label predictor adversarially against a domain discriminator. The feature extractor produces domain-invariant representations to fool the discriminator, which tries to distinguish source from target features. We use negative log likelihood for both classification and domain discrimination losses.}



Figures \ref{fig_adv_results_L}-\ref{fig_adv_results_alpha} confirm \cref{thm_main_result_dann}: required sample sizes grow quadratically as $\Ms, \Ns=O(\numL^2)$ and $\Ms, \Ns = O(\dcom^2)$ with respect to depth and width,\footnote{When studying $\Ms$, we scale feature extractor and label predictor depths together; for $\Ns$, we scale feature extractor and domain discriminator depths together. Width parameter $\dcom$ scales all layer widths proportionally.} and the optimal weight scales as $\alpha_{opt}=O(\sqrt{\Mt})$.
