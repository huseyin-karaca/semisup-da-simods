...
\ifpdf
\hypersetup{
  pdftitle={A Unified Analysis of Generalization and Sample Complexity for Semi-Supervised Domain Adaptation},
  pdfauthor={Elif Vural, Huseyin Karaca}
}
\fi
...
%%% Hüseyin Automated Response Letter Macros
...
% ------------------------------------------------
\newclipboard{article} % manuscript.cpy dosyasını oluşturur

% ------------------------------------------------
% 3. The "Smart" Revision Macro (Tekrar Önleyici)
% ------------------------------------------------
% Kullanım: \paperrevision{1.1} veya \paperrevision{1.2a, 1.2b} veya \paperrevision{2.1, 3.4}
...
%%% Hüseyin Automated Response Letter Macros End
...
\subsubsection{Effect of domain discrepancy on generalization}
A substantial body of theoretical work has studied how domain discrepancy affects target-domain generalization. Ben-David et al.~\cite{BenDavidBCP06, BenDavidBCKPV10} bound the target generalization gap in terms of the VC-dimension of the hypothesis space and the $\mathcal{A}$-distance between source and target distributions.  Their analysis of weighted source-target losses \cite{BenDavidBCKPV10} parallels our study, in that the optimal weight of the target loss should decrease as target labels become scarcer. Mansour et al.~\cite{MansourMR09} present similar findings using Rademacher complexities and a discrepancy measure based on loss-induced disagreements. Following these pioneering works, numerous alternative domain divergence measures have been proposed \cite{RedkoMHS20}, including adversarial Rademacher complexity-based discrepancy \cite{DengGHML23}, margin disparity discrepancy \cite{ZhangLLJ19}, moment-based discrepancy \cite{ZellingerMS21}, margin-aware risks linked to optimal transport \cite{DhouibRL20}, information-theoretic bounds \cite{WangM23, WuMAZ24}, hypothesis-specific divergences \cite{WangM24}, and stochastic predictor-based risks \cite{SiciliaAAH22}. All these works assume that a common classifier is learnt in the original domains without considering domain-aligning transformations. They mainly differ in the discrepancy measure used, while sample complexities follow classical dependence on VC-dimensions or Rademacher complexities. From a domain alignment perspective, these discrepancies can be viewed as post-alignment distances, but many are theoretically defined and difficult to estimate in practice. In contrast, our results in \crefrange{thm:main_result_mmd}{thm_main_result_dann} provide a practical assessment based on the empirical distribution distance computed directly on aligned training data.

\subsubsection{Performance bounds for domain alignment algorithms}
Only a few studies have investigated the performance of learning domain-aligning transformations. Zhou et al.~\cite{ZhouTPT19} propose a multi-task method learning domain similarity through a linear transformation, with estimation error bounded by $O(d_T \sqrt{\log(d_S)/n})$; however, their analysis is limited to linear transformations. Wang and Schneider \cite{WangS15} analyze conditional distribution matching through location and scale transforms, showing the target generalization gap is bounded by $O(1+ 1/\sqrt{\Mt} + \sqrt{\log(\delta^{-1})/(\Ms + \Mt)})$. Fang et al.~\cite{FangLLZ23} consider semi-supervised domain alignment as in our work, but treat the sample complexity as a known parameter rather than deriving it.  Transfer learning, while distinct from domain adaptation, is closely related. Wang et al.~\cite{WangMSX23} analyze transfer and multitask learning through domain-invariant representations, with bounds involving the $\mathcal{Y}$-discrepancy \cite{MohriM12}, which is not easy to estimate in practice. Galanti et al.~\cite{TomerWH16} and Jiao et al.~\cite{JiaoLLY24} study how partial network transfer and shared features improve target convergence. McNamara and Balcan \cite{McNamaraB17} show that in representation learning followed by fine-tuning, the labeled target samples $\Mt$ need only scale with the dimension of the classifier space $\mathcal{H}$, not the more complex composite space $\mathcal{H \circ F}$; a parallel result appears in \cite{TripuraneniJJ20} for multi-task transfer learning. Although our domain adaptation setting differs from these transfer learning studies, they share the focus on handling scarce target labels. Whereas these works tie sample complexity to the richness of the target function class, which can be large for deep networks, our analysis shows that target label scarcity can be tolerated by choosing $\alpha=O(\sqrt{\Mt})$, independently of the target function class complexity.

\subsubsection{Sample complexity of neural networks in a single domain}
Sample complexity of neural networks is well-explored in learning theory \cite{AnthonyB02, BartlettMR21}. The sample complexity of a feed-forward network with $W$ weights and $\numL$ layers scales as $O(W \numL \log(W))$ in terms of the VC-dimension \cite{HarveyLM17}, with $W=\dcom^2 \numL$ for width $\dcom$. This yields $M=O(\dcom^2 \numL^3)$ \cite[Theorem 21.5]{AnthonyB02}, consistent with our domain-adaptive result $\Ms=O(\dcom^2 \numL^2)$ in \cref{thm_main_result_da_mmd,thm_main_result_dann}, with a slightly tighter quadratic dependence on $\numL$. A recent trend is dimension-independent characterization: width dependence has been removed or reduced under norm regularization \cite{NeyshaburTS15, NeyshaburBS18, GolowichRS18, BartlettFT17, WeiM19, VardiSS22, DanielyG24}, though these results crucially rely on dimension-independent bounds on weight matrix norms and yield pessimistic complexities without such assumptions. To the best of our knowledge, our work is the first detailed analysis of the sample complexity of domain-adaptive neural networks. Our analysis does not impose norm constraints on weight matrices; incorporating such constraints could yield tighter bounds, which we leave as future work.
...
\item We share all experimental code at \href{https://github.com/huseyin-karaca/semisup-da}{github.com/huseyin-karaca/semisup-da} publicly to facilitate reproducibility and further research.
\end{itemize}


\cref{sec:exp_results} presents simulation results for the experimental validation of our findings, and \cref{sec:conclusion} concludes the paper. A preliminary version of our study was presented in \cite{Vural18}, which laid the groundwork for the results in \cref{ssec:gen_bnd_arb_dist}.  
...
Let \cref{assum_existence_LLs,assum_HF_comp_Ll_Al} hold. Then for any transformations $\fs \in \Fs$, $\ft \in \Ft$ and hypothesis $\h \in \Hs$, with probability at least
%
\begin{equation}
...
\leq (\N_{\Fs, \epsilon} + \N_{\Ft, \epsilon}) \ O \left( e^{-  N \epsilon^2 } \right) \leq O \left( e^{-  \Mt \epsilon^2 } \right) 
...
However, we do not make these assumptions. We only assume that the distribution discrepancy between $\nus$ and $\nut$ is taken as in \eqref{eq:defn_D_MMD} and the empirical estimate is computed as in \eqref{eq:defn_hD_MMD}.
}
}
...
In order to study the performance of KMM algorithms,  we first derive a bound on the deviation between the actual distribution discrepancy $\D(\fs, \ft)$ and its empirical estimate $\hD(\fs, \ft)$. We make the following assumption on the data distributions:
...
\footnote{
  \paperrevision{1.3}{While \cref{assum_fx_bdd_moments} is stated as a general moment condition, it is naturally satisfied in the context of deep neural networks with bounded input domains. As formally shown later in our analysis of covering numbers (\cref{lem_cov_num_Fs_Ft}), standard neural networks map compact input domains (such as images with bounded pixel values, as defined in \cref{assum_Ax_Atheta}) to bounded feature representations. Since any bounded random variable satisfies the Bernstein moment growth condition, \cref{assum_fx_bdd_moments} holds by definition for the domain-adaptive neural networks studied in \cref{sec_samp_dan} and the datasets (MNIST, MNIST-M) used in \cref{sec:exp_results}.
  }
}
...
Consider a learning algorithm relying on the minimization of a loss function of the form \eqref{eq_obj_learning_mmd} via an MMD-based domain adaptation network. Assume that the classification loss function $\loss$ is bounded by a constant $\bls$ and Lipschitz continuous with respect to the first argument with constant $\Lls$.  Suppose that the source and target data distributions satisfy \cref{assum_existence_LLs,assum_fx_bdd_moments}. Assume also that the network parameters, activation functions  and the kernels satisfy \crefrange{assum_Ax_Atheta}{assum_bnd_act_val_op}. Consider that the weight parameter $\alpha$ in the loss function is chosen such that
%
\[
\alpha
= O \left(
...
\cref{thm_main_result_da_mmd} shows that sample complexities $\Ms$, $\Ns$, and $\Nt$ must increase at rate $O(\dcom^2 \numL^2)$ as network depth $\numL$ and width $\dcom$ increase (ignoring logarithmic terms), indicating quadratic growth with network size to prevent overfitting.
\footnote{The assumption of the existence of constants $\bls$ and $\Lls$ is satisfied in many settings; we derive these for cross-entropy loss in Appendix \ref{sec_app_crossent}.}
For limited labeled target samples $\Mt$, the weight $\alpha$ must shrink at rate $\alpha=O(\sqrt{\Mt})$ to avoid overfitting, and similarly at rate $\alpha=O((\dcom \numL)^{-1})$ as network size grows. Sample sizes scale as $O(\epsilon^{-2})$ for an $O(\epsilon)$ bound on loss difference. 
...
$\loss_\dom: \R \times \R \rightarrow [0, \infty)$ is a domain discrimination loss function; and $\ydoms,  \ydomt \in \R$ denote the domain labels of the source and the target domains. It is common to set the domain discrimination loss $\loss_\dom$ as a logarithmic penalty on the deviation between the estimated domain labels and the true domain labels $\ydoms =0$, $\ydomt =1$ as \cite{GaninUAGLLML16, TzengHSD17, LongC0J18}:
\begin{equation}
...
$ %
%
with the function spaces $\Hs \circ \Fs$ and $\Hs \circ \Ft$ defined\footnote{Note that, the definitions of the function spaces $\Fs, \Ft$ in this section are different from those in \cref{ssec_samp_comp_mmd_net}, as they take different roles between MMD-based and adversarial  networks. Nevertheless, the composite function spaces $\Gs = \Hs \circ \Fs$ and $\Gt = \Hs \circ \Ft$ in this section are the same as those of \cref{ssec_samp_comp_mmd_net}, since the functions $\gs, \gt$ are defined through the classification layer output in both the MMD-based and the adversarial settings.} in \eqref{eq_Gs_Gt_defn_dl}. Here, the features between layers $\lay-1$ and $\lay$ are related as in \eqref{eq_defn_hsl_htl} through the network parameters $\Wsl, \Wtl, \bsl, \btl$ and the nonlinear activation functions $\actl$. 
\footnote{
  \paperrevision{2.5}{Feature extractor networks in common architectures typically consist of several convolutional layers followed by fully connected layers \cite{SinghalWRK23}. In domain adaptation applications, it is standard practice to adopt convolutional layer weights from pretrained networks or to train them using source data \cite{TzengHSD17}. Our theoretical analysis focuses on the fully connected layers of the feature extractor, treating the input source and target samples $\xs, \xt \in \R^{\dz}$ as the responses generated at the output of the convolutional network common between the two domains, as illustrated in \cref{fig_illus_ddan_network}. Notably, our experimental validation in \cref{sec:exp_results} employs complete architectures that include convolutional layers trained end-to-end; the consistency of the experimental results with the theoretical predictions suggests that our bounds remain applicable to practical architectures incorporating convolutional components.
  }
}
...
{Note that \cref{assum_actddan_cont_Lip} is an adaptation of the conditions in \cref{assum_krl_actl_cont,assum_Lk_Leta} to the domain-adversarial setting in consideration.}
...
% We discuss our findings in relation with previous literature. To the best of our knowledge, our study is the first to characterize the sample complexity of domain-adaptive neural networks in depth. Prior work has focused on either the effect of domain discrepancy on generalization, or the sample complexity of neural networks in a single-domain setting. We overview these results below along with relevant studies on domain alignment methods, restating prior findings using our notation. The parameter $\delta$ in bounds signifies that the result holds with probability at least $1-\delta$.

% \subsection{Effect of domain discrepancy on generalization performance}

% Ben-David et al.~\cite{BenDavidBCP06} present one of the earliest analyses of the deviation between source and target distributions. The gap between the expected target loss and the empirical source loss is bounded by
% %
% \[
% O\left(  
...
% \]
% %
% ignoring logarithmic factors, where $\text{dim}_{VC}(\mathcal{H})$ is the VC-dimension of hypothesis space $\mathcal{H}$, $\Ms$ is the number of labeled source samples, and $\lambda$ measures the proximity of the true label function to $\mathcal{H}$. Here $d_{\mathcal{H}}(D_S, D_T)$ is the $\mathcal{A}$-distance \cite{BenDavidBCP06} between $D_S$ and $D_T$:
% %
% \[
%  d_{\mathcal{H}}(D_S, D_T) = 2 \sup_{A \in \mathcal{A}} | P_{D_S}(A) - P_{D_T}(A) |
% \]
% %
% where $\mathcal{A}$ is the set of domain subsets with characteristic functions in $\mathcal{H}$ and $P_{(\cdot)}$ denotes probability.

% This result was extended in \cite{BenDavidBCKPV10} to algorithms minimizing a convex combination of source and target losses, where the hypothesis minimizing the empirical weighted loss generalizes to the target domain within an error of 
% %
% \begin{equation*}
% \begin{split}
% O\bigg(
...
% \end{equation*}
% %
% Here $\hat d_{\mathcal{H}\Delta \mathcal{H}}(D_S, D_T)$ is the empirical divergence over the symmetric difference hypothesis space $\mathcal{H}\Delta \mathcal{H}$ \cite{BenDavidBCKPV10}, $N=\Ns=\Nt$ is the total number of samples in each domain, and $M$ is the total number of labeled samples with $\Ms=(1-\gamma)M$ source and $\Mt=\gamma M$ target samples. This parallels our study: the optimal target loss weight $\alpha$ should decrease as target labels become scarcer (as $\gamma$ decreases), and a high discrepancy $\hat d_{\mathcal{H}\Delta \mathcal{H}}(D_S, D_T)$ drives the loss towards the target by reducing the source weight $1-\alpha$.

% Mansour et al.~\cite{MansourMR09} presented similar findings using Rademacher complexities, characterizing domain deviation through the discrepancy $\text{disc}_{\loss}(D_S, D_T)$, which measures how loss-induced disagreement between hypothesis pairs may differ across $D_S$ and $D_T$.

% Following these works, numerous domain divergence measures have been proposed \cite{RedkoMHS20}. These include a robust discrepancy variant based on adversarial Rademacher complexity \cite{DengGHML23} shown to vary at rate $O(\sqrt{d/M})$ for two-layer ReLU networks, margin disparity discrepancy with bounds via Rademacher complexities and covering numbers \cite{ZhangLLJ19}, moment-based discrepancy with VC-dimension-dependent bounds \cite{ZellingerMS21}, margin-aware risks linked to optimal transport \cite{DhouibRL20}, information-theoretic bounds based on mutual information \cite{WangM23, WuMAZ24}, hypothesis-specific divergences \cite{WangM24}, and stochastic predictor-based risks \cite{SiciliaAAH22}.%
% \footnote{All these works assume a common classifier is learnt in the original domains without considering domain-aligning transformations. They mainly differ in the distribution discrepancy measure used, while sample complexities follow classical dependence on VC-dimensions or Rademacher complexities. From a domain alignment perspective, these discrepancies can be viewed as post-alignment distances, but many are theoretically defined and hard to estimate in practice. In contrast, our results in \crefrange{thm:main_result_mmd}{thm_main_result_dann} provide a practical assessment based on the empirical distribution distance computed directly on aligned training data.}

% % For comparison of Lemma 3 with previous work:
% % The empirical convergence of the MMD estimator was studied in \cite{GrettonBRSS12}, where McDiarmid's inequality is applied to bound the deviation of the biased empirical MMD from the population value. For a characteristic kernel satisfying $0 \leq k(x,x) \leq K$ and equal sample sizes $m = n$, the bound decays at rate $O(\sqrt{K/m})$ with probability improving exponentially in $m\varepsilon^2/K$. Our \cref{lem:bernstein_src_trg} addresses a distinct but related quantity: the deviation of the empirical mean embedding from the true mean embedding in $\X$, considered separately for each domain and for parametric feature transformations $\fs \in \Fs$, $\ft \in \Ft$. The proof uses Yurinskii's Bernstein-type inequality for Hilbert-space-valued random variables \cite{Yurinski76}, which yields an exponential bound governed by the distribution-specific variance and moment parameters $(\vars, \Cs)$, $(\vart, \Ct)$ rather than a uniform kernel bound $K$; consequently, the result is applicable without a bounded-kernel assumption and provides tighter concentration when $\vars$ and $\vart$ are small. Moreover, this per-domain formulation is the key building block for the uniform bound on $|\D(\fs,\ft) - \hD(\fs,\ft)|$ over all transformation pairs $(\fs, \ft) \in \Fs \times \Ft$ established in \cref{lem:unif_bnd_D_hD}, which does not have a direct counterpart in \cite{GrettonBRSS12} where the kernel is fixed rather than learned.

% \subsection{Performance bounds for domain alignment algorithms}

% Very few theoretical analyses have investigated learning domain-aligning transformations. Zhou et al.~\cite{ZhouTPT19} propose a multi-task method learning domain similarity through a linear transformation $\mathbf{G}$, with estimation error bounded by $O(d_T \sqrt{\log(d_S)/n})$ under projection incoherence, where $d_S$, $d_T$ are the domain dimensions and $n$ is the number of tasks; however, their analysis is limited to linear transformations.

% Wang and Schneider \cite{WangS15} analyze conditional distribution matching, showing the target generalization gap is bounded by
% %
% \[
% O\left (  1+ \frac{1}{\sqrt{\Mt}} + \sqrt{\frac{\log(\delta^{-1})}{\Ms + \Mt}} \right)
% \]
% %
% when the source domain is mapped to the target through a location and scale transform. 

% Fang et al.~\cite{FangLLZ23} consider semi-supervised domain alignment as in our work, but do not explore the sample complexity of learning domain transformations; they instead treat it as a known parameter and aim to show that labeled target data requirements can be alleviated under certain assumptions.
% %

% Transfer learning, while different from domain adaptation, is closely related. Wang et al.~\cite{WangMSX23} analyze transfer and multitask learning through domain-invariant representations, showing the source-target performance gap varies at rate
% %
% \[
% O\left(\text{dist}_{\mathcal{Y}}(\fs, \ft) + \sqrt{ \frac{\log(\delta^{-1})}{\Ms + \Mt}} \right),
% \]
% %
% where $\text{dist}_{\mathcal{Y}}(\fs, \ft)$ is the $\mathcal{Y}$-discrepancy \cite{MohriM12} between the transformed domains, which is not easy to estimate in practice. Galanti et al.~\cite{TomerWH16} model transfer learning where a target task and multiple source tasks are drawn from the same meta-distribution, showing that the number of source tasks and samples per task must scale with the number of edges in the transferred and target-specific network components, respectively. Jiao et al.~\cite{JiaoLLY24} distinguish shared and domain-specific features in deep transfer learning, showing that cross-task transferability improves target convergence rates. McNamara and Balcan \cite{McNamaraB17} analyze representation learning followed by fine-tuning, with the source-to-target performance gap bounded by $O(\sqrt{\text{dim}_{VC}(\mathcal{H \circ F}) / \Ms} + \sqrt{\text{dim}_{VC}(\mathcal{H})/\Mt})$, where $\mathcal{F}$ is the representation space and $\mathcal{H}$ is the classifier space. The key insight is that $\Mt$ scales with the dimension of $\mathcal{H}$ alone, not the more complex $\mathcal{H \circ F}$. A parallel result in \cite{TripuraneniJJ20} shows that in multi-task transfer learning, the labeled samples for a new task scale only with its task-specific map complexity, given sufficient data for previous tasks.%
% \footnote{Although our domain adaptation setting differs from these transfer learning studies, they share the focus on handling scarce target labels. Whereas these works tie sample complexity to the richness of the target function class, which can be large for deep networks, our analysis shows that target label scarcity can be tolerated by choosing $\alpha=O(\sqrt{\Mt})$, independently of the target function class complexity.}


% \subsection{Sample complexity of neural networks in a single domain}

% Sample complexity of neural networks is well-explored in learning theory; see \cite{AnthonyB02, BartlettMR21} for comprehensive overviews. Although this line of research is limited to the single-domain setting, we briefly review these results for comparison with our domain-adaptive bounds.

% The sample complexity of a feed-forward network with $W$ weights, $\numL$ layers, and $s$ output units with fixed piecewise-polynomial activations is \cite[Theorem 21.5]{AnthonyB02}
% %
% \begin{equation}
...
% \end{equation}
% %
% for error $\epsilon$. With network width $\dcom$, we have $W= \dcom^2 \numL$, giving sample complexity $M=O(\dcom^2 \numL^3)$ in \eqref{eq_samp_comp_Bartlett_NN}---quadratic in $\dcom$ and cubic in $\numL$. This is consistent with our results in \cref{thm_main_result_da_mmd,thm_main_result_dann}, where $\Ms= O(\dcom^2 \numL^2)$, with a slightly tighter quadratic dependence on $\numL$.

% A recent trend is dimension-independent characterization of sample complexity. \cite{NeyshaburTS15} show exponential dependence on depth but remove width dependence under group norm regularization. Subsequent works reduce the dependence on network size to polynomial \cite{WeiM19}, quadratic \cite{NeyshaburBS18}, linear \cite{GolowichRS18}, and logarithmic \cite{BartlettFT17} factors. Harvey et al.~\cite{HarveyLM17} show the VC-dimension of ReLU networks is $O(W \numL \log(W))$, yielding comparable bounds to ours. More recently, width dependence has been removed for one-layer networks \cite{VardiSS22} and reduced to logarithmic factors for two-layer networks \cite{DanielyG24} under bounded Frobenius and spectral norm constraints. These results crucially rely on dimension-independent bounds on weight matrix norms and yield pessimistic complexities without such assumptions.%
% \footnote{While these studies have comprehensively advanced the understanding of neural network classifiers, they all focus on the single-domain scenario. To the best of our knowledge, our work is the first detailed analysis of the sample complexity of domain-adaptive neural networks. Our analysis does not impose norm constraints on weight matrices; incorporating such constraints could yield tighter bounds, which we leave as future work.}
...
In this section, we present experimental results for the verification of the proposed generalization bounds. We first study the generic bounds using shallow classifiers, then examine the sample complexity of domain-adaptive neural networks. 
% \footnote{
Complete experimental details are provided in \cref{sec_detailed_exp_results} of the supplement, and the codes are shared publicly in \url{https://github.com/huseyin-karaca/semisup-da}.
% }  
...
\label{ssec_exp_syn_data}
We first validate the findings of \cref{sec:gen_bounds} on a synthetic two-class data set.
% \footnote{
Source and target data sets are generated by applying two different geometric transformations to 400 samples drawn from the standard normal distribution in $\R^2$. We emulate a setting where the transformations $\fs$ and $\ft$ are learnt from data with some error: $\fs$ and $\ft$ are formed by perturbing the ground truth geometric transformations with a transformation estimation error $\tau$, and we test a range of error levels $\tau$. Target misclassification rates are evaluated over 1000 test samples drawn from the target distribution and classified through the learnt hypothesis $\wvect$ and target transformation $\ft$.
% }
The classifier trained in the common domain is a regularized ridge regression solving
%
\vspace{-0.3cm}
\begin{align}
...
\cref{fig:toy_error_Mt} shows the variation of the target misclassification rate with $\Mt$ for different values of $\alpha$. Recall from \cref{thm:gen_defect_target} that the expected target loss $\Lt(\ft, \h)$ deviates from its reference value by an amount of $\epsilon$; for this to hold with fixed probability, the term $\Mt \epsilon^2$ in \eqref{eq_prob_expr_thm1} must remain constant, 
%\footnote{Ignoring logarithmic factors and assuming that the covering numbers in \cref{thm:gen_defect_target} grow at a typical geometric rate with decreasing covering radius.} 
implying $\epsilon = O(\sqrt{1/\Mt})$. The dashed lines in \cref{fig:toy_error_Mt} correspond to fitted rates of decay $O(\sqrt{1/\Mt})$, which closely match the experimental data, confirming the prediction of \cref{thm:gen_defect_target}. We also observe that large $\Mt$ values favor larger $\alpha$, while $\alpha$ must be chosen smaller at small $\Mt$ values, aligning with the theoretical scaling $\alpha = O(\sqrt{\Mt})$.

\cref{fig:toy_error_tau} shows the variation of the target error with the transformation estimation error $\tau$, which is proportional to the distribution distance $\D(\fs, \ft)$. The misclassification rate increases approximately linearly with $\tau$, as confirmed by the fitted dashed lines, which is coherent with the prediction of \cref{thm:gen_defect_target} that the expected target loss grows proportionally to $\D(\fs, \ft)$.


\subsubsection{Experiments with the MIT-CBCL face data}
We next experiment on the MIT-CBCL face data set \cite{MITCBCL}
% \footnote{The dataset contains 3240 synthetic face images of 10 subjects rendered under 36 illumination conditions and 9 poses.}
(\cref{fig:face_dataset}).
Using Pose 1 as the source domain and Poses 2, 5, 9 as separate target domains, we compute source-target mappings via the PCA-based method of \cite{FernandoHST13} and train an SVM classifier on all labeled samples from both domains.

\cref{fig:mit_error_Mt,fig:mit_error_Ms} show that the misclassification rates of unlabeled target samples decrease effectively with increasing labeled samples. The rates of decrease are consistent with $O(\sqrt{1/\Mt})$ and $O(\sqrt{1/\Ms})$ as predicted by the theory; the fitted theoretical curves (dashed lines) closely match the experimental data.
...
\begin{figure}[t!]
  ...
  \vspace{-0.3cm}
  \caption{Sample images from the MIT-CBCL face data set for four different subjects, rendered respectively under poses 1, 2, 5, and 9 for various illumination conditions. The dataset contains 3240 synthetic face images of 10 subjects rendered under 36 illumination conditions and 9 poses. 
  %We use Pose 1 (frontal) as source and Poses 2, 5, 9 as targets in separate trials.
  }
...
We next verify the sample complexity results of \cref{thm_main_result_da_mmd,thm_main_result_dann} for domain-adaptive neural networks. We use the MNIST data set \cite{mnist} as the source domain and MNIST-M \cite{mnistm} as the target domain.\footnote{MNIST consists of 60000 images and MNIST-M contains 59000 handwritten digit images with colored backgrounds. Networks are trained with labeled and unlabeled samples from both domains, and the target accuracy is defined as the correct classification rate on target test samples. In all experiments, hyperparameters are chosen to keep the network in the overfitting regime, enabling the characterization of sample complexity. Complete details and code for reproducing all experiments are provided in \cref{ssec_exp_dan_detailed}.} We present results for MMD-based (\cref{ssec_exp_mmd_dan}) and adversarial (\cref{exp_adv_da}) architectures, characterizing sample complexity with respect to depth $\numL$, width $\dcom$. We additionally investigate the optimal value of the weight $\alpha$ of the target loss in the objective function for both cases.
...
We adopt the architecture from \cite{LongCWJ15}, building on our previous study \cite{KaracaAAAAUV23}.\footnote{The network begins with convolutional layers followed by fully connected MMD layers with coupled parameters between domains and equal widths. \paperrevision{2.4a}{Dropout and batch normalization are applied after each layer.} We use the PyTorch implementation in \cite{mmd_code_pytorch}.} The network minimizes
%
\begin{equation} 
...
\end{equation} 
%
where $\loss(\cdot, \cdot)$ is the cross-entropy loss and $\fs=\ft=f$, $\fsl=\ftl=f^l$.




In \cref{fig_mmd_results_L}, we study the sample complexity with respect to network depth $\numL$. \cref{fig_mmd-layer-Ms-linefit,fig_mmd-layer-Ns-linefit} show the decrease in target accuracy as $\numL$ increases for different $\Ms$ and $\Ns$ values. For several reference accuracy levels, we identify the smallest $\Ms$ and $\Ns$ values ensuring each target accuracy as $\numL$ grows,
% \footnote{In cases where obtaining the exact value of $\numL$ exceeded our computational resources, we resorted to linear extrapolation of the curves in \cref{fig_mmd-layer-Ms-linefit,fig_mmd-layer-Ns-linefit} to approximately infer the corresponding $\numL$ value.} 
which are plotted in \cref{fig_mmd-layer-Ms-quadprog,fig_mmd-layer-Ns-quadprog}. The fitted quadratic polynomials confirm the prediction of \cref{thm_main_result_da_mmd} that sample complexities grow at rates $\Ms=O(\numL^2)$ and $\Ns=O(\numL^2)$. \cref{fig_mmd_results_d} demonstrates analogous quadratic growth $\Ms=O(\dcom^2)$ with respect to the width parameter $\dcom$, which scales the width of all MMD layers, consistently with \cref{thm_main_result_da_mmd}.


\cref{fig_mmd_results_alpha} validates the prediction of \cref{thm_main_result_da_mmd} for the optimal weight parameter. The target accuracy in \cref{fig_mmd-optalpha_quadfit_Ms234} exhibits a non-monotonic variation with $\alpha$ as expected. The optimal value $\alpha_{opt}$, identified via polynomial fitting for each $\Mt$, is plotted in \cref{fig_mmd-optalpha-asqrtx_fit_den_Mss}. The experimental data is consistent with the fitted $O(\sqrt{\Mt})$ curves, supporting the theoretical scaling $\alpha_{opt}=O(\sqrt{\Mt})$.


\subsubsection{Adversarial domain adaptation networks}
\label{exp_adv_da}

We adopt the domain-adversarial architecture from \cite{GaninUAGLLML16}, adapted for our semi-supervised setting.
% \footnote{We use the PyTorch implementation in \cite{fungtion_DANN_py3}, modified to support semi-supervised domain adaptation.} 
...
\left(
 ...
 +
  ...
  \end{split}
\end{equation*} 
where $\loss$ and $\loss_\dom$ are the negative log likelihood functions and $\fs=\ft=f$.


The feature extractor consists of convolutional layers, while the label predictor and domain discriminator are fully connected.\footnote{When studying $\Ms$, the feature extractor and label predictor depths are set equal as $\numL$; for $\Ns$, the feature extractor and domain discriminator depths are equated. The width parameter $\dcom$ scales both convolutional channels and fully connected layer widths proportionally. \paperrevision{2.4b}{Batch normalization and ReLU layers are included after each layer.} }

\cref{fig_adv_results_L,fig_adv_results_d} present the sample complexity with respect to network depth $\numL$ and width $\dcom$. Following the same methodology as in \cref{ssec_exp_mmd_dan}, the results confirm the predictions of \cref{thm_main_result_dann}: required sample sizes grow quadratically as $\Ms, \Ns=O(\numL^2)$ and $\Ms, \Ns = O(\dcom^2)$. \cref{fig_adv_results_alpha} further validates that the optimal weight parameter scales as $\alpha_{opt}=O(\sqrt{\Mt})$, consistently across different $\Ms$ values.
...
\begin{figure}[t!]
  \centering
    \subfloat[]%
      ...
      \hspace{0.1\textwidth}
    \subfloat[]%
      {\label{fig_mmd-layer-Ms-quadprog}\includegraphics[width = 0.4\textwidth]{figures/mmd-layer-Ms-quadprog.pdf}}\\ \vspace{-0.45cm}
    \subfloat[]%
...
\begin{figure}[t!]
  \begin{center}
       \subfloat[]
         ...
         \hspace{0.1\textwidth}
       \subfloat[]
         {\label{fig_adv-layer-Ms-quadprog}\includegraphics[width = 0.4\textwidth]{figures/adv-layer-Ms-quadprog.pdf}}\\ \vspace{-0.6cm}
        \subfloat[]
...
\begin{center}
     \subfloat[]
       ...
       \hspace{0.1\textwidth}
     \subfloat[]
       {\label{fig_adv-dcm-Ms-quadprog}\includegraphics[width = 0.4\textwidth]{figures/adv-dcm-Ms-quadprog.pdf}}\\ \vspace{-0.45cm}
            \subfloat[]
